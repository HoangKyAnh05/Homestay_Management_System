(() => {
  // Cache auto-scan flag immediately before SPA router rewrites the URL
  try {
  ...
  console.log('[Lá Đỏ TikTok AI] Universal TikTok & TikTok Studio Content Script Active (v1.0.5).');
  ...
  function pollAndHighlightTikTokComment(commentText, authorName) {
    ...
    const pollTimer = setInterval(() => {
      ...
      for (const node of commentNodes) {
        ...
        if (cleanAuthor && cleanAuthor !== 'người dùng tiktok' && t.includes(cleanAuthor)) {
          targetComment = node;
          break;
        }
      }
      ...
      if (!targetComment) {
        const rows = Array.from(document.querySelectorAll('tr, [role="row"], li, div[class*="item"], div[class*="row"]'));
        for (const r of rows) {
      ...
      if (targetComment) {
        ...
        const highlightTarget = targetComment.closest('tr, [role="row"], li') || targetComment;
        highlightTarget.scrollIntoView({ behavior: 'smooth', block: 'center' });
        highlightTarget.style.transition = 'all 0.4s ease';
        highlightTarget.style.outline = '3px solid #fe0979';
        highlightTarget.style.boxShadow = '0 0 28px rgba(254, 9, 121, 0.75)';
        highlightTarget.style.backgroundColor = 'rgba(254, 9, 121, 0.15)';
        ...
        showInPageToast(`👀 Đã định vị bình luận của ${authorName || 'khách'}: "${(clean || authorName || '').slice(0, 30)}..."`);
      } else if (tries >= 25) {
        clearInterval(pollTimer);
      }
    }, 400);
  ...
  function injectAiButtonsToTikTokComments() {
    ...
    // Strategy B: Find any active INLINE reply input bar (like "Add a reply...")
    const inlineReplyInputs = Array.from(document.querySelectorAll(
      'input[placeholder*="reply" i], input[placeholder*="trả lời" i], textarea[placeholder*="reply" i], textarea[placeholder*="trả lời" i], div[placeholder*="reply" i], div[placeholder*="trả lời" i], div[data-placeholder*="reply" i], div[class*="ReplyInput"]'
    ));

    inlineReplyInputs.forEach(inputEl => {
      const container = inputEl.closest('div[class*="container" i], div[class*="wrapper" i], div[class*="Input" i], form') || inputEl.parentElement;
      if (!container || container.querySelector('.lado-inline-reply-ai-btn')) return;

      const submitBtn = container.querySelector('button, div[role="button"], svg');
      
      const inlineAiBtn = document.createElement('button');
      inlineAiBtn.type = 'button';
      inlineAiBtn.className = 'lado-main-ai-btn lado-inline-reply-ai-btn';
      inlineAiBtn.innerHTML = `<span>✨ Trả lời AI</span>`;
      inlineAiBtn.title = 'Tự động sinh câu trả lời AI cho bình luận này';

      inlineAiBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        e.preventDefault();

        inlineAiBtn.classList.add('loading');
        inlineAiBtn.innerHTML = `<span>⏳ Đang sinh & gửi...</span>`;

        const commentParent = container.closest('div[data-e2e*="comment"], div[class*="CommentItem"], div[class*="commentItem"], div[class*="commentRow"], li') || container.previousElementSibling;
        const info = extractTikTokComment(commentParent) || { authorName: 'Khách', message: '' };
        const videoTitle = getTikTokVideoTitle();
        ...
        chrome.runtime.sendMessage({
          ...
          payload: {
            postTitle: videoTitle,
            postContent: 'Lá Đỏ Homestay Sa Pa view thung lũng Mường Hoa & cafe săn mây',
            commentText: info.message || 'Tư vấn đặt phòng Lá Đỏ Homestay',
            authorName: info.authorName || 'Khách',
            tone: 'WARM'
          }
        }, (response) => {
          inlineAiBtn.classList.remove('loading');
          inlineAiBtn.innerHTML = `<span>✨ Trả lời AI</span>`;

          if (response && response.reply) {
            const boundedReply = enforceTikTokCharLimit(response.reply, 135);
            fillAndSubmitTikTokComment(container, boundedReply);
          }
        });
      });

      if (submitBtn && submitBtn.parentElement === container) {
        container.insertBefore(inlineAiBtn, submitBtn);
      } else {
        container.appendChild(inlineAiBtn);
      }
    });
  }

  // 8. Inject AI Comment Generator Button at the MAIN Video Comment Box (Bottom of Video)
  function injectMainVideoAiCommentButton() {
    if (document.querySelector('.lado-main-ai-btn:not(.lado-inline-reply-ai-btn)')) return;

    const mainPostBtn = document.querySelector('button[data-e2e="comment-post"]') ||
                        document.querySelector('div[data-e2e="comment-post"]') ||
                        document.querySelector('button[class*="commentPost"]') ||
                        document.querySelector('button[class*="SubmitButton"]') ||
                        document.querySelector('div[class*="DivPostButton"]');

    const mainInputContainer = document.querySelector('div[data-e2e="comment-input"]') ||
                               document.querySelector('div[class*="DivCommentInputContainer"]') ||
                               document.querySelector('div[class*="CommentInputContainer"]') ||
                               document.querySelector('div[class*="BottomComment"]');

    const targetParent = mainPostBtn?.parentNode || mainInputContainer;
    if (!targetParent) return;

    const mainAiBtn = document.createElement('button');
    mainAiBtn.type = 'button';
    mainAiBtn.className = 'lado-main-ai-btn';
    ...
    mainAiBtn.addEventListener('click', async (e) => {
      ...
      chrome.runtime.sendMessage({
        ...
        payload: {
          ...
          commentText: 'Hãy viết một bình luận ngắn gọn, thân thiện, bắt trend giới thiệu hoặc tương tác chào mọi người về Lá Đỏ Homestay Sa Pa trên video này.',
          authorName: 'Khán giả',
          tone: 'WARM'
        }
      }, (response) => {
        ...
        if (response && response.reply) {
          const boundedReply = enforceTikTokCharLimit(response.reply, 135);
          fillAndSubmitTikTokComment(null, boundedReply);
        }
      });
    });

    if (mainPostBtn) {
      mainPostBtn.parentNode.insertBefore(mainAiBtn, mainPostBtn);
    } else {
      targetParent.appendChild(mainAiBtn);
    }
  }

  /**
   * Universal TikTok Input Filler & Submitter (Safe for TikTok Studio & Web)
   */
  function fillAndSubmitTikTokComment(commentContainer, replyText) {
    ...
    const fillInterval = setInterval(() => {
      ...
      if (!inputEl) {
        inputEl = document.querySelector('div[contenteditable="true"], div[data-e2e="comment-input"] div[contenteditable], textarea[placeholder*="comment" i], textarea[placeholder*="bình luận" i], textarea[placeholder*="reply" i], div[role="textbox"]');
      }
      ...
      if (inputEl) {
        ...
        setTimeout(() => {
          let submitBtn = null;
          if (commentContainer) {
            submitBtn = commentContainer.querySelector('button[type="submit"], div[data-e2e="comment-post"], button[class*="post" i], button[class*="send" i], button[class*="submit" i]');
          }
          if (!submitBtn) {
            submitBtn = document.querySelector('div[data-e2e="comment-post"], button[data-e2e="comment-post"], button[class*="SubmitButton"], div[class*="DivPostButton"], button[class*="post" i]');
          }
  ...
  function checkAndTriggerAutoScanTikTok(force = false) {
    ...
    let isAutoScan = force || urlParams.get('lado_auto_scan') === 'true' || window.location.hash.includes('lado_auto_scan=true');
    let isReset = urlParams.get('reset') === 'true' || window.location.hash.includes('reset=true');

    try {
      if (sessionStorage.getItem('lado_auto_scan_tt') === 'true') isAutoScan = true;
      if (sessionStorage.getItem('lado_reset_tt') === 'true') isReset = true;
    } catch (e) {}
