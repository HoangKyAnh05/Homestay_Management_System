// Lá Đỏ Homestay TikTok AI - Popup Controller
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function isOwnComment(c) {
    if (!c || !c.authorName) return false;
    const name = (c.authorName || '').trim().toLowerCase();

    const brandKeywords = ['lá đỏ homestay sa pa', 'la do homestay official', 'lado official', 'quản trị viên homestay'];
    if (brandKeywords.some(kw => name === kw || name.startsWith(kw))) return true;
  ...
  async function handleScanActiveTab() {
    ...
    if (!tab.url.includes('tiktok.com')) {
      alert('Vui lòng mở tab Video TikTok hoặc TikTok Studio trước khi bấm quét bình luận!');
      return;
    }
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'SCAN_COMMENTS' }, (response) => {
      if (btnScanTop) btnScanTop.innerHTML = `<span>🔍 Quét bình luận TikTok</span>`;
      if (btnScanEmpty) btnScanEmpty.innerHTML = `<span>🔍 Quét bình luận TikTok ngay</span>`;

      if (chrome.runtime.lastError) {
        alert('Không thể kết nối với tab TikTok này. Vui lòng F5 làm mới tab TikTok rồi bấm quét lại!');
        ...
        // For the active TikTok video/page, replace existing comments with the fresh unreplied comments
        ...
        if (freshComments.length === 0) {
          alert('Không tìm thấy bình luận nào trên video/trang TikTok này.');
        }
      }
    });
  }
  ...
  function renderCommentsList(comments) {
    ...
    comments.forEach((item, idx) => {
      ...
      card.innerHTML = `
        <div class="comment-header">
          <div class="author-info">
            ...
            <div>
              <span class="author-name">${escapeHtml(item.authorName || 'Người dùng TikTok')}</span>
              <span class="platform-tag tt">🎵 TikTok</span>
            </div>
          </div>
          <div style="display: flex; align-items: center; gap: 6px;">
            <button class="btn-link-post" id="btn-view-post-${idx}" title="Mở video TikTok hoặc định vị bình luận này">
        ...
        <div class="ai-reply-container">
          <div class="ai-controls-row">
            <select class="tone-select-mini" id="tone-select-${idx}">
              <option value="WARM" ${item.tone === 'WARM' ? 'selected' : ''}>🌿 Trẻ trung & Bắt trend</option>
              <option value="BOOKING_INQUIRY" ${item.tone === 'BOOKING_INQUIRY' ? 'selected' : ''}>📅 Báo giá phòng & Bio</option>
              <option value="GRATITUDE" ${item.tone === 'GRATITUDE' ? 'selected' : ''}>❤️ Thả tim & Cảm ơn</option>
              <option value="PROMO" ${item.tone === 'PROMO' ? 'selected' : ''}>🎁 Ưu đãi 10% TikTok</option>
            </select>
      ...
      // Function to navigate and highlight comment on TikTok
      async function navigateToTikTokComment(e) {
        if (e) {
          e.preventDefault();
          e.stopPropagation();
        }
        ...
        const activeTab = allTabs.find(t => t.active && t.url && t.url.includes('tiktok.com'));

        let targetUrl = item.postUrl;
        const isPublicVideoUrl = targetUrl && (targetUrl.includes('/video/') || targetUrl.includes('/photo/') || targetUrl.includes('/@'));
        ...
        // If we have an active tab on TikTok Studio or TikTok
        if (activeTab && activeTab.id) {
          // Send direct highlight message to current tab
          ...
          // If targetUrl is a specific public video that is DIFFERENT from the active tab (e.g. active is Studio)
          if (isPublicVideoUrl && !activeTab.url.includes(targetUrl.split('?')[0])) {
            chrome.tabs.create({ url: targetUrl, active: true });
            return;
          }

          // Otherwise, focus the active tab
          await chrome.tabs.update(activeTab.id, { active: true });
          if (activeTab.windowId) {
            await chrome.windows.update(activeTab.windowId, { focused: true });
          }
        } else {
          // If no active TikTok tab, open targetUrl or default tiktok.com
      ...
      if (btnViewPost) {
        btnViewPost.addEventListener('click', navigateToTikTokComment);
      }

      // Event: Click "👀 Kiểm tra"
      const btnCheckReply = card.querySelector(`#btn-check-reply-${idx}`);
      if (btnCheckReply) {
        btnCheckReply.addEventListener('click', navigateToTikTokComment);
      }
      ...
      if (btnAiGen) {
        btnAiGen.addEventListener('click', async () => {
          ...
          chrome.runtime.sendMessage({
            ...
            payload: {
              postTitle: item.videoTitle || currentVideoTitle || 'Video TikTok Lá Đỏ Homestay Sa Pa',
              postContent: 'Lá Đỏ Homestay Sa Pa view thung lũng Mường Hoa & cafe săn mây cực chill',
              commentText: item.message,
              authorName: item.authorName,
              tone: selectedTone
            }
