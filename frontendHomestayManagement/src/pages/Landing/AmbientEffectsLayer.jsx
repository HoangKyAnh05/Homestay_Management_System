...
export default function AmbientEffectsLayer() {
  ...
  useEffect(() => {
    ...
    const frameInterval = 1000 / 36; // Cap to silky smooth 36 FPS for minimal CPU/GPU load

    const isLowSpec = (navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4) || window.innerWidth < 768;
    ...
    const fireflyCount = isLowSpec ? 14 : 24;
    ...
    const leaves = Array.from({ length: leafCount }, () => ({
      ...
      fallSpeed: Math.random() * 0.7 + 0.5,
      color: Math.random() > 0.4 ? 'rgba(217, 72, 54, ' : 'rgba(234, 88, 12, ', // Crimson / Vermilion
      opacity: Math.random() * 0.35 + 0.45,
    }));

    // Draw single stylized maple leaf (Optimized without heavy CPU shadowBlur)
    ...
    const render = (now) => {
      ...
      fireflies.forEach((f) => {
        ...
        if (dist < 100 && dist > 0) {
          ...
          f.x += (dx / dist) * force + mouse.vx * 0.08;
          f.y += (dy / dist) * force + mouse.vy * 0.08;
        }
        ...
        if (f.alpha > 0.03) {
          ctx.save();
          // Glow halo
          const gradient = ctx.createRadialGradient(f.x, f.y, 0, f.x, f.y, f.radius * 3.0);
          gradient.addColorStop(0, `hsla(${f.hue}, 95%, 75%, ${f.alpha})`);
          gradient.addColorStop(0.5, `hsla(${f.hue}, 90%, 60%, ${f.alpha * 0.4})`);
          gradient.addColorStop(1, `hsla(${f.hue}, 90%, 50%, 0)`);

          ctx.fillStyle = gradient;
          ctx.beginPath();
          ctx.arc(f.x, f.y, f.radius * 3.0, 0, Math.PI * 2);
          ctx.fill();
      ...
      leaves.forEach((leaf) => {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 120 && dist > 0) {
          const wind = (1 - dist / 120) * 1.6;
          leaf.x += (dx / dist) * wind + mouse.vx * 0.12;
          leaf.y += (dy / dist) * wind + mouse.vy * 0.12;
          leaf.angularSpeed += (Math.random() - 0.5) * 0.03;
        }
