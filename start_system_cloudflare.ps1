...
Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Where-Object { $_.LocalPort -in 8080, 5173 } | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }
Start-Sleep -Seconds 1

# 2. Khoi dong Backend Spring Boot (:8080)
...
# 3. Khoi dong Frontend Vite (:5173)
...
# 4. Cho ca Frontend (:5173) va Backend (:8080) san sang
for ($i = 0; $i -lt 30; $i++) {
    Start-Sleep -Seconds 1
...
# 5. Khoi dong Cloudflare Tunnel cho Web Frontend (:5173)
...
# 6. Trich xuat link deploy Cloudflare
$urlWeb = $null
 ...
 * Dung luong vo han, truy cap muot ma tren tat ca trinh duyet.
