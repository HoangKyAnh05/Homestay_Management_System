(() => {
  ...
  function pollAndHighlightYouTubeComment(commentText, authorName) {
    ...
    const pollTimer = setInterval(() => {
      ...
      const textNodes = deepQuerySelectorAll(
        '#content-text, .yt-core-attributed-string, yt-formatted-string, #comment-content, ytcp-comment #body #content, yt-formatted-string#content',
        document
      );
      ...
      for (const node of textNodes) {
        ...
        if (t.includes(clean) || (clean.length >= 3 && clean.includes(t))) {
      ...
      if (targetThread) {
        ...
        targetThread.style.outline = '4px solid #ea580c';
        targetThread.style.boxShadow = '0 0 28px rgba(234, 88, 12, 0.9)';
        targetThread.style.backgroundColor = 'rgba(234, 88, 12, 0.18)';
        targetThread.style.borderRadius = '10px';
        setTimeout(() => {
          targetThread.style.outline = '';
          targetThread.style.boxShadow = '';
          targetThread.style.backgroundColor = '';
        }, 7000);
        showInPageToast(`👀 Đã định vị bình luận của ${authorName || 'khán giả'}: "${clean.slice(0, 35)}..."`);
      } else if (tries >= 30) {
        clearInterval(pollTimer);
      }
    }, 300);
  ...
  function checkAndTriggerAutoScanYouTube(force = false) {
    ...
    let isAutoScan = force || url.includes('lado_auto_scan=true') || url.includes('lado_auto_scan');
    let isReset = url.includes('reset=true');

    try {
      if (sessionStorage.getItem('lado_auto_scan_yt') === 'true') isAutoScan = true;
      if (sessionStorage.getItem('lado_reset_yt') === 'true') isReset = true;
    } catch (e) {}

    if (!isAutoScan || window.__LADO_YT_AUTO_SCAN_EXECUTED__) return;
    ...
    if (isReset && typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      chrome.storage.local.remove(['scannedCommentsList', 'scannedCount'], () => {
        console.log('[Lá Đỏ YouTube AI] Đã xóa dữ liệu quét cũ.');
      });
    }
    ...
    (async () => {
      try {
        ...
        // Initial wait for YouTube Studio API & Polymer cards
  ...
  setTimeout(checkAndTriggerAutoScanYouTube, 1000);
  setTimeout(checkAndTriggerAutoScanYouTube, 3000);
})();
