// Lá Đỏ Homestay YouTube AI - Service Worker (Background Script)

const DEFAULT_GROQ_KEY = '';

chrome.runtime.onInstalled.addListener(() => {
  console.log('[Lá Đỏ YouTube AI Extension] Installed successfully.');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'GET_CONFIG') {
    chrome.storage.local.get(['serverUrl', 'groqApiKey', 'aiTone', 'jwtToken', 'enableYoutubeButton', 'autoSubmit'], (config) => {
      sendResponse(config);
    });
    return true;
  }
...
async function handleSyncScannedComments(payload) {
  ...
  console.log('[Lá Đỏ YouTube AI] Background syncing comments to backend:', serverUrl, payload?.comments?.length);
...
async function handleGenerateAiReply({ postTitle, postContent, commentText, tone, authorName }) {
  ...
  if (apiKey && apiKey.startsWith('gsk_')) {
    try {
      ...
      console.warn('[Lá Đỏ YouTube AI] Direct Groq API failed, trying Backend:', err);
    }
  }
  ...
  try {
    ...
    console.warn('[Lá Đỏ YouTube AI] Backend request failed:', err);
  }
...
4. ĐỊNH DẠNG: Chỉ trả về nội dung câu trả lời thuần túy, không có dấu ngoặc kép bọc ngoài.`;
  ...
  const userPrompt = `Tên khách: ${authorName || 'Khán giả'}
...
function generateLocalSmartReply(commentText, tone, authorName) {
  ...
  const namePrefix = authorName && authorName !== 'Khán giả' && authorName !== 'Khách' ? `Chào bạn ${authorName}! ` : 'Dạ chào bạn! ';
