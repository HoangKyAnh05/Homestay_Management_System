...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const finalStepNumber = workflowMode === 'fast' ? 3 : workflowMode === 'quality' ? 6 : 5;
    ...
    <header className="h-16 px-4 sm:px-6 border-b border-gray-800/80 bg-[#0B0F19]/90 backdrop-blur-md flex items-center justify-between z-30 sticky top-0 gap-3">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <input
          ...
          className="bg-gray-800/60 hover:bg-gray-800/90 focus:bg-gray-800 border border-indigo-500/40 focus:border-indigo-400 rounded-lg px-3.5 py-1.5 text-xs sm:text-sm font-semibold text-indigo-200 focus:text-white focus:outline-none transition-all w-48 sm:w-64 md:w-80 lg:w-96 max-w-full shadow-inner"
          placeholder="Tên video: VD Săn mây Sa Pa 2N1Đ..."
        />
      </div>

      {/* Center: Workflow Combo Guide Selector with Shiny Badges */}
      <div className="hidden lg:flex items-center gap-1 bg-gray-900/90 p-1 rounded-2xl border border-indigo-500/30 shadow-lg shadow-indigo-950/40">
        ...
        >
          ...
          <span>🎙️ Kịch Bản & Giọng (1-5)</span>
        </button>
      </div>
