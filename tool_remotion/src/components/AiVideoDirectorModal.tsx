...
export const AiVideoDirectorModal: React.FC<AiVideoDirectorModalProps> = ({
  ...
  // State tạo tự động bằng Gemini AI
  ...
  const handleGenerateWithAi = async (forceSemantic = false) => {
    ...
    try {
      ...
      setAiGenError(err.message || 'Lỗi khi gọi Gemini AI. Vui lòng kiểm tra lại API Key hoặc thử bộ sinh thông minh!');
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/80 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Cấu hình Google Gemini API Key để sinh kịch bản tự động bằng AI Cloud"
            >
              <Key className="w-3.5 h-3.5" />
              <span>{geminiApiKey.trim() ? '🟢 Gemini AI Cloud' : '⚡ AI Ngữ Cảnh Tự Động'}</span>
        ...
        {/* Thanh Cấu Hình Gemini API Key (Khi mở) */}
        {showApiKeyConfig && (
          <div className="bg-slate-900 text-white px-6 py-3.5 border-b border-slate-800 space-y-2.5 animate-in slide-in-from-top-2 duration-150">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-100">
                  Cấu hình Google Gemini API Key (AI Cloud Sinh Kịch Bản Đạo Diễn):
                </span>
              </div>
              <a
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                placeholder="Dán Gemini API Key dạng AIzaSy... vào đây..."
            ...
            {isKeySavedToast && (
              <div className="text-[11px] text-emerald-400 font-medium flex items-center gap-1">
                ...
                <span>Đã lưu cấu hình Gemini API Key vào hệ thống thành công!</span>
              </div>
            )}
          </div>
        )}
